...
export function JobsProvider({ children }: { children: React.ReactNode }) {
  ...
  const transformJobs = async (jobsData: any[]): Promise<Job[]> => {
    ...
    return jobsData.map(job => {
      ...
      // Get application count if available
      const applications_count = job.applications_count || 0;
  ...
  const fetchJobs = async (filters?: JobFilters): Promise<Job[]> => {
    try {
      ...
      const transformedJobs = await transformJobs(data || []);
      setJobs(transformedJobs);
      
      // Set featured jobs (newest 5)
